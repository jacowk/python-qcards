...
class ReviewGui:
    ...
    def __init__(self, main_window):
        ...
        review_stack_button.grid(row=1, column=0, padx=5, pady=10)
        refresh_button = ttk.Button(self.button_frame, text="Refresh", command=self.refresh_table)
        refresh_button.grid(row=1, column=1, padx=5, pady=10)
    ...
    def review_stack(self):
        ...
        # Open the review gui
        review_stack_gui = ReviewCardsGui(self.stack_window, stack, cards_for_review)
...
class ReviewCardsGui:
    ...
    def __init__(self, main_review_list_window, stack, cards_for_review):
        ...
        self.review_cards_window = tk.Toplevel(self.main_review_list_window)
        self.review_cards_window.transient(self.main_review_list_window)
        self.review_cards_window.title("Review Cards")
        self.review_cards_window.resizable(False, False)

        self.frame = tk.Frame(self.review_cards_window)
        ...
        # Buttons
        self.reveal_back_button = tk.Button(self.button_frame, text="Reveal Back", command=self.reveal_back_content)
        self.reveal_back_button.grid(column=0, row=0)
        self.next_card_button = tk.Button(self.button_frame, text="Next Card", command=self.next_card)
        self.next_card_button.grid(column=1, row=0)
        self.close_button = tk.Button(self.button_frame, text="Close", command=self.review_cards_window.destroy)
        self.close_button.grid(column=2, row=0)

        self.review_cards_window.wait_visibility()
        self.review_cards_window.grab_set()

    def calculate_screen_position(self, x, y):
        gui_util = u.QCardsGUIUtil()
        screen_coordinates = gui_util.calculate_window_center(x, y, self.review_cards_window.winfo_screenwidth(), self.review_cards_window.winfo_screenheight())
        self.review_cards_window.geometry("{}x{}+{}+{}".format(x, y, screen_coordinates[0], screen_coordinates[1]))
